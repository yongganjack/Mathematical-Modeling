"""Question 5 configuration, serialization, and protected result3 export."""
...
def q5_config(config: Mapping[str, Any] | None) -> dict[str, Any]:
    """Resolve the compact quick profile and a larger competition profile."""
    ...
    defaults: dict[str, Any] = {
        ...
        "pso_particles": 18 if quick else 36,
        "stage1_iterations": 14 if quick else 32,
        "stage2_iterations": 14 if quick else 32,
...
def export_result3(
    ...
    """Write B:C,E:L only, retaining official result3 identifiers and notes."""

    ordered = sorted(plans, key=lambda plan: plan.uav_index)
    if [plan.uav_index for plan in ordered] != list(range(5)):
        raise ValueError("result3 export requires exactly FY1 through FY5")
    if any(len(plan.bombs) > 3 for plan in ordered):
        raise ValueError("each UAV may export at most three bombs")
    ...
    if not validation["valid"]:
        raise RuntimeError("result3.xlsx export validation failed")

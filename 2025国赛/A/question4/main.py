...
def run(args: argparse.Namespace) -> tuple[int, Path]:
    ...
    try:
        ...
        print("\n最优投放策略：")
        for plan, item in zip(plans, derived):
            ...
            print(f"  烟幕干扰弹投放点: {[float(value) for value in item.release_point]} m")
            print(f"  烟幕干扰弹起爆点: {[float(value) for value in item.explosion_point]} m")

"""English PNG/SVG plots for Question 2."""
...
def plot_trajectory(data: Any, plan: Any, derived: Any, output_dir: str | Path) -> dict[str, Path]:
    ...
    ax.plot([start[0], end[0]], [start[1], end[1]], label="UAV trajectory"); ax.scatter([end[0]], [end[1]], label="Explosion"); ax.set_xlabel("x"); ax.set_ylabel("y"); ax.set_title("Question 2 trajectory"); ax.legend()
...
def plot_intervals(intervals: Any, output_dir: str | Path) -> dict[str, Path]:
    ...
    ax.set_xlabel("time (s)"); ax.set_ylabel("missile index"); ax.set_title("Smoke coverage intervals"); _save(fig, output_dir / "intervals"); return {"png": output_dir / "intervals.png", "svg": output_dir / "intervals.svg"}
...
def plot_optimizer_history(history: list[dict[str, Any]], output_dir: str | Path) -> dict[str, Path]:
    ...
    ax.set_xlabel("evaluations"); ax.set_ylabel("best duration (s)"); ax.set_title("PSO vs DE history"); ax.legend(); _save(fig, output_dir / "optimization_history")
